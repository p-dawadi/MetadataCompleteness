...
# Step 1 of the pipeline: retrieve candidate Streptococcus anginosus 16S
# rRNA gene records from NCBI Nucleotide.
#
# This is your original retrieval script (the one you uploaded/added to
# the GitHub repo). The query logic, organism string, and length
# thresholds are UNCHANGED from what actually produced the 350-record
# curated dataset already analyzed in the manuscript -- re-running with
# identical settings is the point (to document, not to redo, the
# retrieval). Do not change QUERY, MIN_LEN_FULL, or MAX_LEN_FULL.
#
# WHY THIS VERSION IS DIFFERENT ("fireproof")
# --------------------------------------------
# Your network keeps dropping the GenBank download mid-transfer
# (IncompleteRead), at similar byte counts (~500KB-2.3MB) regardless of
# how many records are in the batch -- that pattern points to something
# on your network path (a campus/corporate proxy, antivirus HTTPS
# inspection, or a flaky VPN) killing long-lived or large downloads,
# not a problem with the query itself. Splitting batches in half wasn't
# enough because the trigger seems to be duration/size of an individual
# connection, not record count.
#
# This version fixes that at the root instead of working around it:
#   1. Fetches ONE record at a time. Each single-record download is
#      small and fast, so it's very unlikely to hit whatever is cutting
#      off larger/longer transfers.
#   2. Uses `requests` with a proper connection-level retry policy
#      (urllib3 Retry: 5 attempts, exponential backoff, retries on
#      connection errors and 5xx responses) instead of raw urllib --
#      requests/urllib3 handle interrupted chunked-transfer responses
#      far more gracefully than the low-level http.client calls
#      Biopython's Entrez module uses underneath.
#   3. Is fully resumable / crash-proof: every successfully fetched
#      record is immediately appended to genbank_cache.gb and its ID
#      recorded in fetched_ids.txt. If the script is interrupted or
#      crashes for ANY reason, just run it again -- it skips everything
#      already fetched and continues from where it left off. Nothing is
#      ever lost, and nothing is ever re-downloaded.
#   4. A record that fails every retry is logged to failed_ids.txt
#      instead of crashing the whole run; you'll get a clear summary at
#      the end and can just re-run the script again to retry only those.
#
# None of this changes the query, the organism, or the length filter --
# only how robustly the same retrieval is carried out over the network.
#
# Install dependencies:
#   pip install --upgrade biopython pandas requests
#
# Run (safe to interrupt with Ctrl+C and re-run at any time):
#   python 1_retrieve_sequences.py
#
# When it finishes, please send me (or paste) the summary it prints at
# the end -- especially:
#   1) "Found N nucleotide IDs ..."          <- already have this: 2047
#   2) "Saved N sequences to Sp_all_ge200bp.csv ..."
#   3) "Saved N sequences to Sp_1200_1700bp.csv ..."  <- should be 350
#      if nothing on NCBI has changed since your original January 2026
#      run; if it's not exactly 350, send me the new Sp_1200_1700bp.csv
#      too so I can see what changed.
